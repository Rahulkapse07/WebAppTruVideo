...
public class HomePageTest extends BaseTest {
	...
	public void homePageSetup() {
	  ...
	  @Test(priority = 1) public void verify_RepairOrder_HeaderTab_Working() {
	  ...
	  @Test(priority = 2) public void verify_OrdersMessage_HeaderTab_Working()
	  throws Exception {
	  Assert.assertTrue(homepage.clickOn_Order_MessagesHeader()); page.goBack(); }
	  
	  @Test(priority = 3) public void verify_Prospect_HeaderTab_Working() { String
	  actualTitle = homepage.clickOn_Prospect_Header();
	  Assert.assertEquals(actualTitle, AppConstants.PROSPECT_PAGE_TITLE);
	  page.goBack(); }
	  
	  @Test(priority = 4) public void verify_ProspectsMessage_HeaderTab_Working() {
	  Assert.assertTrue(homepage.clickOn_Prospect_MessagesHeader()); page.goBack();
	  }
	  
	  @Test(priority = 5) public void verify_Reminder_HeaderTab_Working() { String
	  actualTitle = homepage.clickOn_Reminder_Header();
	  Assert.assertEquals(actualTitle, AppConstants.REMINDER_PAGE_URL);
	  page.goBack(); }
	  
	  @Test(priority = 6) public void verify_Training_HeaderTab_Working() { String
	  actualTitle = homepage.clickOn_Training_Header();
	  Assert.assertEquals(actualTitle, AppConstants.TRAINING_PAGE_TITLE);
	  page.goBack(); }
	  
	  @Test(priority = 7) public void verify_User_HeaderTab_Working() { String
	  actualTitle = homepage.clickOn_User_Header();
	  Assert.assertEquals(actualTitle, AppConstants.USER_PAGE_TITLE);
	  page.goBack(); }
	  
	  @Test(priority = 8) public void verify_Contact_HeaderTab_Working() { String
	  actualTitle = homepage.clickOn_ContactList_Header();
	  Assert.assertEquals(actualTitle, AppConstants.CONTACT_LIST_PAGE_TITLE);
	  page.goBack(); }
	  
	  @Test(priority = 9) public void
	  verifyDealerTab_UnderOrgainizationHeader_Working() { String actualTitle =
	  homepage.clickOnDealersHeaderTab(); Assert.assertEquals(actualTitle,
	  AppConstants.DEALERS_PAGE_TITLE); page.goBack(); }
	  
	  @Test(priority = 10) public void
	  verifyDealerGroupTab_UnderOrgainizationHeader_Working() { String actualTitle
	  = homepage.clickOnDealerGroupHeaderTab(); Assert.assertEquals(actualTitle,
	  AppConstants.DEALER_GROUP_PAGE_TITLE); page.goBack(); }
	  
	  @Test(priority = 11) public void
	  verifyUserGroupTab_UnderOrgainizationHeader_Working() { String actualTitle =
	  homepage.clickOnUserGroupHeaderTab(); Assert.assertEquals(actualTitle,
	  AppConstants.USER_GROUP_PAGE_TITLE); page.goBack(); }
	  
	  @Test(priority = 12) public void
	  verifySavedVideoLibraryTab_UnderOrgainizationHeader_Working() { String
	  actualTitle = homepage.clickOnSAvedVideoLibraryHeaderTab();
	  Assert.assertEquals(actualTitle, AppConstants.SAVED_VIDEO_PAGE_TITLE);
	  page.goBack(); }
	  
	  @Test(priority = 13) public void verifyDevicesTab_UnderSystemHeader_Working()
	  { String actualTitle = homepage.clickOnDevicesHeaderTab();
	  Assert.assertEquals(actualTitle, AppConstants.DEVICES_PAGE_TITLE);
	  page.goBack(); }
	  
	  @Test(priority = 14) public void verify_Open_Close_OnAdvanceSearchWindow() {
	  ...
	  @Test(priority = 18,description ="WA-5580,5581") public void
	  verify_DateRangeFilter_OnAdvanceSearchWindow() {
	  Assert.assertTrue(homepage.checkRepairOrdersWithinDateRange()
	  ,"Searched Data is NOT Within Date range");
	  
	  }
	  
	  @Test(priority = 19) public void verify_SearchFilter_OnAdvanceSearchWindow()
...
//	  }
	  ...
	  homepage.clickOnAccountSettingTextButton(); Assert.assertEquals(actualTitle,
	  AppConstants.ACCOUNT_SETTING_PAGE_TITLE); page.goBack(); page.reload(); }
	  
	  @Test(priority = 26) public void verify_SwitchDealerFunction() {
